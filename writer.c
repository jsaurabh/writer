...
#define WRITER_VERSION "0.0.1"
#define TAB_STOP 8
#define QUIT_CONFIRM 1
#define HL_HIGHLIGHT (1<<0)
...
struct editorSyntax HLDB[] = {
  {
    ...
    C_HL_keywords,
    "//", "/*", "*/",
    HL_HIGHLIGHT || HL_HIGHLIGHT_STRINGS
  },
};
...
void editorUpdateSyntax(erow *row){
  ...
  while (i < row->rsize){
    ...
    if (E.syntax->flags && HL_HIGHLIGHT){
    ...
    if (filerow >= E.numrows) {
      if(E.numrows == 0 && y == E.screenrows / 3){
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
		      "Writer -- version %s", WRITER_VERSION);
    ...
    case CTRL_KEY('q'):
      if (E.dirty && times > 0){
        editorSetStatusMessage("WARNING! File has unsaved changes."
			       "Press Ctrl-Q %d more times to quit.", times);
	times--;
	return;
      }
      write(STDOUT_FILENO, "\x1b[2J", 4);
